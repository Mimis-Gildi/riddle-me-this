...
Everyone else overhypes the latest LLM achievements even when they've never ran any.
...
It's the way the Three-Plebs-Collective orients around the resulting wealth distribution.
From the Emperor's perspective, it doesn't matter how many prosperous empires are there; it is always the 1st person view.
And Engineering collective will always conform to the size of the market regardless of the efficacy formula
-- and that is the middle class. Considering the current projection that we have, <5%, the middle class must shrink.
This is exactly the formula we saw repeat since the dot-com bubble.
The only way to increase the size of the middle class is to increase the size of the economy.
And to do that, it is necessary to have larger and broader adoption of the enabling factors, i.e., the AI.
More capable contributors mean more GDP growth.
Another way to look at this is that production of automobiles did not make automobile a more expensive option,
it made the horse a cheaper alternative. Personal AI Augmentation does exactly the same thing.
