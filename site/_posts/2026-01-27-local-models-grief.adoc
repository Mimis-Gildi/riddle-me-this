...
ifndef::env-site[:imagesdir: ../assets/images/models/]
ifdef::env-site[:imagesdir: {site-baseurl}/assets/images/models/]
...
:nvidia-dgx-spark: https://www.nvidia.com/en-us/products/workstations/dgx-spark/[NVIDIA DGX Spark]
:ollama-context-length: https://docs.ollama.com/context-length[Increase it]
:qwen3-30b-thinking: https://huggingface.co/Qwen/Qwen3-30B-A3B-Thinking-2507[Qwen3-30B-A3B-Thinking-2507]

.Did you notice 'Claude Code {plus} local models' became a hot topic overnight?
image::local-model-corporate-reality.png[Local Model versus Hosted Model]
{nbsp}
...
{nvidia-dgx-spark} is about $4k per unit (ballpark) and needs a place to live.
...
image::ollama-claude-tested-tools.png[Minimal set of IDEs.]
{nbsp}
...
image::ollama-home-claude.png[ollama for Claude Code]
{nbsp}
...
image::ollama-claude-no-claude.png[Claude Code Operates properly]
{nbsp}
...
image::ollama-claude-thinks-its-gpt.png[Claude Code Operates properly]
{nbsp}
...
image::ollama-claude-qwen3-coder.png[Qwen3-coder tiny]
{nbsp}

Currently `ollama` default context window is 4096. That's nothing, really.
{ollama-context-length} as the docs say, or with `/set parameter num_ctx`.
...
image::ollama-local-load.png[Mac GPU always loaded]
{nbsp}
...
We're going to use {qwen3-30b-thinking} because:
...
The core conclusion doesn't change: the box stays pegged, model capability is still bounded, and headroom for extensions is limited.
But I learned something useful: even under corporate constraints, you *can* experiment with identity/memory techniques
-- as long as you treat it as research and keep your data boundary and approvals clean.
